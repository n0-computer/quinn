...
impl SendStream {
    ...
    ///
    /// It is an error to write to a [`SendStream`] after `finish`ing it. [`reset`](Self::reset) may
    /// still be called after `finish` to abandon transmission of any stream data that might still
    /// be buffered.
    ///
    /// To wait for the peer to receive all buffered stream data, see [`stopped`](Self::stopped).
    ///
    /// May fail if [`finish`](Self::finish) or [`reset`](Self::reset) was previously called. This
    /// error is harmless and serves only to indicate that the caller may have incorrect assumptions
    /// about the stream's state.
    ...
    /// when the stream is [`finish`](Self::finish)ed locally and all stream data has been received
    /// (but not necessarily processed) by the peer, after which it is no longer meaningful for the
