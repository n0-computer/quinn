...
impl Paths {
    ...
    fn path_status(&self, path_id: PathId) -> Result<PathStatus, ClosedPath> {
        self.path(path_id)
            .map(PathData::local_status)
            .ok_or(ClosedPath { _private: () })
    }

    fn path_remote_address(&self, path_id: PathId) -> Result<SocketAddr, ClosedPath> {
        self.path(path_id)
    ...
    fn set_path_status(
        ...
        path_id: PathId,
        status: PathStatus,
    ) -> Result<Option<PathStatus>, SetPathStatusError> {
        let path = self
            .path_mut(path_id)
            .ok_or(SetPathStatusError::ClosedPath)?;

        let prev = path.status.local_update(status);
        Ok(prev)
    }

    fn get_mut(&mut self, path_id: &PathId) -> Option<&mut PathState> {
        self.paths.get_mut(path_id)
    }

    fn get(&self, path_id: &PathId) -> Option<&PathState> {
        self.paths.get(path_id)
    }

    fn remove(&mut self, path_id: PathId) -> Option<PathStats> {
        self.paths.remove(&path_id);
        self.path_stats.remove(&path_id)
    }

    fn iter(&self) -> impl Iterator<Item = (&PathId, &PathState)> {
        self.paths.iter()
    }

    fn keys(&self) -> impl Iterator<Item = &PathId> {
        self.paths.keys()
    }

    fn values(&self) -> impl Iterator<Item = &PathState> {
        self.paths.values()
    }
    ...
    fn contains_path(&self, path_id: &PathId) -> bool {
    ...
    fn open_path_ensure(
            ...
            .find(|(_id, path)| path.data.remote == remote)
        {
            ...
            None => self
                .open_path(remote, initial_status, now, peer_params)
                .map(|id| (id, false)),
        }
    }
    ...
    fn open_path(
        ...
        if self.rem_cids.get(&path_id).map(CidQueue::active).is_none() {
            return Err(PathError::RemoteCidsExhausted { path_id });
        }

        let path = self.ensure_path(path_id, remote, now, peer_params);
    ...
    fn max_path_id(&self) -> PathId {
        self.remote_max_path_id.min(self.local_max_path_id)
    }

    fn set_max_concurrent_paths(&mut self, count: NonZeroU32) -> PathId {
        ...
        self.local_max_path_id.saturating_add(extra_needed)
    }

    fn set_remote_max_path_id(&mut self, path_id: PathId) -> bool {
    ...
    fn on_paths_blocked(&self, max_path_id: PathId) -> Result<(), TransportError> {
    ...
    fn on_path_cids_blocked(&self, path_id: PathId) -> Result<(), TransportError> {
    ...
    fn set_max_path_id(&mut self, max_path_id: PathId) -> bool {
    ...
    fn issue_first_path_cids(&mut self) -> impl Iterator<Item = PathId> {
        let max_path_id = self.max_path_id();
        let path_id = self.max_path_id_with_cids.next();
        self.max_path_id_with_cids = max_path_id;

        path_id.range(&max_path_id)
    }
    ...
    fn close_path(
        ...
        path_id: PathId,
        pto: Duration,
    ) -> Result<PathId, ClosePathError> {
        if self.abandoned_paths.contains(&path_id)
            || path_id > self.max_path_id()
            || !self.paths.contains_key(&path_id)
        {
        ...
        let next_max_path_id = self.local_max_path_id.saturating_add(1u8);
        Ok(next_max_path_id)
    }
    ...
    fn ensure_path(
        ...
        let peer_max_udp_payload_size =
            u16::try_from(peer_params.max_udp_payload_size.into_inner()).unwrap_or(u16::MAX);
    ...
    fn migrate(
        ...
        if let Some(report) = observed_addr {
            if let Some(updated) = new_path.update_observed_addr_report(report) {
                ...
                event = Some(Event::Path(PathEvent::ObservedAddr {
                    id: path_id,
                    addr: updated,
                }));
            }
    ...
    fn path_stats(&mut self, path_id: PathId) -> Option<PathStats> {
...
impl Connection {
    pub(crate) fn new(
        ...
        let mut this = Self {
            ...
            local_cid_state,
            paths: Paths::new(now, remote, rem_cid, allow_mtud, config.clone()),
            local_ip,
            state,
    ...
    pub fn open_path_ensure(
        ...
        self.paths
            .open_path_ensure(remote, initial_status, now, &self.peer_params)
    }
    ...
    pub fn open_path(
        ...
        match self
            .paths
            .open_path(remote, initial_status, now, &self.peer_params)
        {
            ...
            Err(PathError::RemoteCidsExhausted { path_id }) => {
    ...
    pub fn close_path(
        ...
        let next_max_path_id = self.paths.close_path(now, path_id, pto)?;
        ...
        self.set_max_path_id(now, next_max_path_id);
    ...
    pub fn set_path_status(
        ...
        let prev = self.paths.set_path_status(path_id, status)?;
        if prev.is_some() {
            self.spaces[SpaceId::Data]
                .pending
                .path_status
    ...
    fn ensure_path(
        ...
        let data = self
            .paths
            .ensure_path(path_id, remote, now, &self.peer_params);
    ...
    pub fn poll_transmit(
        ...
        let have_available_path = self.paths.iter().any(|(id, path)| {
            path.data.validated
                ...
                && self.paths.rem_cids.contains_key(id)
        });
        ...
        loop {
            ...
            let Some(remote_cid) = self.paths.rem_cids.get(&path_id).map(CidQueue::active) else {
                let err = PathError::RemoteCidsExhausted { path_id };
                if !self.paths.abandoned_paths.contains(&path_id) {
                ...
                let next_path_id = self.paths.keys().find(|&&next| next > path_id).copied();
                match next_path_id {
                    Some(next_path_id) => {
        ...
        if transmit.is_empty() && self.state.is_established() {
            ...
            let probe_data = loop {
                ...
                let active_cid = self.paths.rem_cids.get(&path_id).map(CidQueue::active);
                let eligible = self.path_data(path_id).validated
                    && !self.path_data(path_id).is_validating_path()
                    && !self.paths.abandoned_paths.contains(&path_id);
    ...
    pub fn set_max_concurrent_paths(
        ...
        let new_max_path_id = self.paths.set_max_concurrent_paths(count);
    ...
    fn set_max_path_id(&mut self, now: Instant, max_path_id: PathId) {
        if self.paths.set_max_path_id(max_path_id) {
    ...
    fn inner_on_ack_received(
        ...
        if self.paths.abandoned_paths.contains(&path) {
    ...
    fn discard_path(&mut self, path_id: PathId, now: Instant) {
        ...
        let path_stats = self.paths.remove(path_id).unwrap_or_default();
    ...
    fn handle_lost_packets(
        ...
        if let Some(largest_lost) = lost_packets.last().cloned() {
            ...
            let largest_lost_sent = self.spaces[pn_space]
                ...
                .unwrap()
                .time_sent;
            let path_stats = self.paths.path_stats.entry(path_id).or_default();
            ...
            Ok((packet, number)) => {
                ...
                if number.zip(dedup).is_some_and(|(n, d)| d.insert(n)) {
                    ...
                    if !self.state.is_closed() {
                        ...
                        if self.side().is_server() && !self.paths.abandoned_paths.contains(&path_id)
                        {
                            // Only the client is allowed to open paths
                            self.ensure_path(path_id, remote, now, number);
                        }
                        if self.paths.contains_path(&path_id) {
                            self.on_packet_authenticated(
                                now,
                                packet.header.space(),
                                path_id,
        ...
        for result in frame::Iter::new(payload)? {
            ...
            match frame {
                ...
                Frame::RetireConnectionId(frame::RetireConnectionId { path_id, sequence }) => {
                    ...
                    match self.local_cid_state.get_mut(&path_id) {
                        ...
                        Some(cid_state) => {
                            ...
                            let has_path = !self.paths.abandoned_paths.contains(&path_id);
                ...
                Frame::MaxPathId(frame::MaxPathId(path_id)) => {
                    ...
                    if self.paths.set_remote_max_path_id(path_id) {
                        self.issue_first_path_cids(now);
                    }
                }
                Frame::PathsBlocked(frame::PathsBlocked(max_path_id)) => {
                    ...
                    if self.is_multipath_negotiated() {
                        self.paths.on_paths_blocked(max_path_id)?;
                    } else {
                        return Err(TransportError::PROTOCOL_VIOLATION(
                ...
                Frame::PathCidsBlocked(frame::PathCidsBlocked { path_id, next_seq }) => {
                    ...
                    // Ref <https://www.ietf.org/archive/id/draft-ietf-quic-multipath-14.html#name-paths_blocked-and-path_cids>
                    if self.is_multipath_negotiated() {
                        self.paths.on_path_cids_blocked(path_id)?;
                        if next_seq.0
                            > self
                                .local_cid_state
                                .get(&path_id)
    ...
    fn migrate(
        ...
        if let Some(event) =
            self.paths
                .migrate(path_id, now, remote, observed_addr, &self.peer_params)
        {
    ...
    fn issue_first_path_cids(&mut self, now: Instant) {
        for path_id in self.paths.issue_first_path_cids() {
            self.endpoint_events
    ...
    fn populate_packet(
        ...
        {
            let frame = frame::PathsBlocked(self.paths.remote_max_path_id);
            frame.encode(buf);
            qlog.frame(&Frame::PathsBlocked(frame));
            space.pending.paths_blocked = false;
            sent.retransmits.get_or_create().paths_blocked = true;
            trace!(max_path_id = ?self.paths.remote_max_path_id, "PATHS_BLOCKED");
        ...
        while space_id == SpaceId::Data && frame::PathCidsBlocked::SIZE_BOUND <= buf.remaining_mut()
        {
            ...
            let next_seq = match self.paths.rem_cids.get(&path_id) {
    ...
    fn set_peer_params(&mut self, params: TransportParameters) {
        ...
        if let Some(ref info) = params.preferred_address {
            ...
            self.paths.rem_cids.get_mut(&PathId::ZERO).expect("not yet abandoned").insert(frame::NewConnectionId {
                path_id: None,
                sequence: 1,
        ...
        if let (Some(local_max_path_id), Some(remote_max_path_id)) = (
            ...
            self.paths.local_max_path_id = local_max_path_id;
            self.paths.remote_max_path_id = remote_max_path_id;
    ...
    fn path(&self, path_id: PathId) -> Option<&PathData> {
        self.paths.path(path_id)
    }

    fn path_mut(&mut self, path_id: PathId) -> Option<&mut PathData> {
        self.paths.path_mut(path_id)
    }

    fn path_data(&self, path_id: PathId) -> &PathData {
        self.paths.path_data(path_id)
    }

    fn path_data_mut(&mut self, path_id: PathId) -> &mut PathData {
        self.paths.path_data_mut(path_id)
    }

    /// Check the current active remote CID sequence for `PathId::ZERO`
    #[cfg(test)]
    pub(crate) fn active_rem_cid_seq(&self) -> u64 {
        self.paths.active_rem_cid_seq()
    }

    /// Returns the detected maximum udp payload size for the current path
    #[cfg(test)]
    pub(crate) fn path_mtu(&self, path_id: PathId) -> u16 {
        self.paths.path_mtu(path_id)
    }

    /// Triggers path validation on all paths
    #[cfg(test)]
    pub(crate) fn trigger_path_validation(&mut self) {
        self.paths.trigger_path_validation();
    }
...
pub enum PathError {
    ...
    #[error("remoted CIDs exhausted: blocked {path_id}")]
    RemoteCidsExhausted {
        /// The path id that was blocked because of exhaustion
        path_id: PathId,
    },
